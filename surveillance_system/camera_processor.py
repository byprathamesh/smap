...
class CameraProcessor:
        ...
        while self.running:
            try:
                ...
                while self.running and self.cap and self.cap.isOpened():
                    ...
                    print(f"DEBUG_CP: Starting AI analysis for camera {self.camera_id}")
                    detections = self.analyzer.analyze_frame(frame)
                    print(f"DEBUG_CP: AI returned {len(detections)} detections")
                    
                    # Step 2: Calculate threat score using Risk Scoring Algorithm
                    if detections:
                        print(f"DEBUG_CP: Processing {len(detections)} detections for threat analysis")
                        threat_score = self._analyze_threat_level(frame, detections)
                        print(f"DEBUG_CP: Final detections before display (with all flags): {detections}")
                    else:
                        # Reset lone woman tracking if no people detected
                        self._reset_lone_woman_tracking()
                        threat_score = 0
                        print(f"DEBUG_CP: No detections found, threat score = 0")
                    
                    # Step 3: Display frame with all visualizations
                    print(f"DEBUG_CP: Displaying frame with threat score: {threat_score}")
    ...
    def _display_frame_with_overlay(self, frame, threat_score, detections=None):
        ...
        """
        try:
            # **PART 2 FIX**: Verify detections content immediately before drawing
            print(f"DEBUG_OVERLAY: _display_frame_with_overlay received {len(detections) if detections else 0} detections.")
            if detections and len(detections) > 0:
                ...
                for i, det in enumerate(detections[:3]): # Print up to 3 detections
                    print(f"DEBUG_OVERLAY: Example detection {i+1}: {det}")
            ...
            if detections:
                print(f"DEBUG_CP: Drawing {len(detections)} detection overlays")
                for i, person in enumerate(detections):
                    # NEW DEBUG: Log each detection being drawn
                    print(f"DEBUG_OV: Drawing for detection: {person}")
                    try:
                        # Extract bounding box coordinates
                        x, y, w, h = person['box']
                        print(f"DEBUG_CP: Drawing detection {i}: box=({x}, {y}, {w}, {h})")
                        
                        # Determine box color based on status
                        box_color = (0, 255, 0)  # Default green
                        if person.get('distress', False):
                            box_color = (0, 0, 255)  # Red for distress
                            print(f"DEBUG_CP: Detection {i} has distress - using red box")
                        elif person.get('surrounded', False):
                            box_color = (0, 165, 255)  # Orange for surrounded
                            print(f"DEBUG_CP: Detection {i} has surrounded flag - using orange box")
                        ...
                        print(f"DEBUG_CP: Successfully drew detection {i}")
                        
                    except Exception as e:
                        print(f"DEBUG_CP: Error drawing person detection {i}: {e}")
                        continue
            else:
                print(f"DEBUG_CP: No detections to draw")
            ...
            if self._frame_save_counter % 50 == 0:
                ...
                print(f"DEBUG_SAVE: Saved frame_with_overlay_{self._frame_save_counter:05d}_{timestamp}.jpg to {debug_output_dir}")
            ...
            try:
                ...
                frame[:] = display_frame
                print(f"DEBUG_CP: Updated frame with overlays for main.py display")
            except cv2.error as e:
    ...
    def _calculate_risk_score(self, detections, context=None):
        """
        Calculates a dynamic risk score based on detections and context.
        ...
        for det in detections:
            ...
            # Check if any female is surrounded (avoid counting multiple times)
            if not surrounded_detected:
                if det.get('gender', '').lower() in ['woman', 'female']:
                    # Check if this female is surrounded by counting nearby males
        ...
        # --- Context Multiplier ---
        # For now, we'll use a placeholder. This could be expanded with time of day.
        context_multiplier = context.get('location_risk', 1.0)
        
        # Add night time multiplier
        if is_night_time():
    ...
    def get_frame(self):
        ...
        if self.analyzer and self.analyzer.is_ready():
            ...
            print(f"DEBUG_CP: AIAnalyzer returned {len(detections)} detections.")
            print(f"DEBUG_CP: Detections before threat analysis: {detections}")
        ...
        print(f"DEBUG_CP: Detections before overlay drawing: {detections}")
